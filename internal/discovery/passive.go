...
// ScanPassive listens for network traffic to identify hosts
// This is completely passive and doesn't generate any traffic
func ScanPassive(iface network.NetworkInfo, duration time.Duration) (PassiveDiscoveryResults, error) {
	start := time.Now()
	...
	var (
		discoveredHosts = make(map[string]network.HostStatus)
		mu              sync.Mutex
...
func processPacket(packet gopacket.Packet, mu *sync.Mutex,
	hosts map[string]network.HostStatus, ipCounter map[string]int, localIP string) {
	...
	if ipLayer != nil {
		...
		if ip.SrcIP.String() != localIP && isInLocalNetwork(ip.SrcIP.String()) {
			...
			if ipCounter[srcIP] >= 3 {
				...
				// Save the host
				hosts[srcIP] = network.HostStatus{
					IPAddress:  srcIP,
					MACAddress: macStr,
					IsAlive:    true,
				}
		...
		if !ip.DstIP.Equal(net.IPv4bcast) && ip.DstIP.String() != localIP && isInLocalNetwork(ip.DstIP.String()) {
			...
			if ipCounter[dstIP] >= 2 {
				...
				hosts[dstIP] = network.HostStatus{
					IPAddress:  dstIP,
					MACAddress: "unknown",
					IsAlive:    true,
				}
	...
	if ipv6Layer != nil {
		...
		if strings.HasPrefix(srcIPv6, "fe80::") {
			...
			if ipCounter[srcIPv6] >= 2 {
				...
				// Save the host
				hosts[srcIPv6] = network.HostStatus{
					IPAddress:  srcIPv6,
					MACAddress: macStr,
					IsAlive:    true,
				}
