...
// ScanSYN performs a SYN scan on critical ports to find hosts that didn't respond to other scans
// This is more stealthy and can sometimes bypass firewalls
func ScanSYN(hosts []network.HostStatus, iface network.NetworkInfo) ([]network.HostStatus, error) {
	scanStart := time.Now()
	defer func() {
	...
	var deadHosts []network.Host
	for _, host := range hosts {
		if !host.IsAlive {
			deadHosts = append(deadHosts, network.Host{IPAddress: host.IPAddress})
		}
	}
