...
// GetNetworkHosts returns all valid host IP addresses in the given CIDR network
// It excludes the network address and broadcast address
func GetNetworkHosts(networkCIDR string) ([]Host, error) {
...
// PrintNetworkHosts gets all hosts in the primary network and prints them
func PrintNetworkHosts() {
	...
	hosts, err := GetNetworkHosts(primary.NetworkCIDR)
	if err != nil {
